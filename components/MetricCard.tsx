...
import { LucideIcon, BedDouble, ChevronUp, ChevronDown, Maximize2, Minimize2, CheckCircle2, Circle, Flame, Footprints, HeartPulse } from 'lucide-react';
...
const MetricCard: React.FC<MetricCardProps> = ({ 
  ...
  isExpanded, onToggleExpand, tasks = [], extraDetails
}) => {
  ...
  // Format value to max 2 decimal places, removing trailing zeros (e.g. 5.00 -> 5, 4.742 -> 4.74)
      ...
      <div className="relative z-10 flex flex-col h-full pointer-events-none">
        ...
        <div className={`flex flex-col ${isExpanded ? 'md:flex-row gap-8 mt-4' : 'justify-center flex-1 mt-2'}`}>
          ...
          {isExpanded && (
            ...
            >
              {/* Display Extra Details if available (For Vitals) */}
              {extraDetails ? (
                <div className="grid grid-cols-2 gap-3 mb-4">
                   <div className={`p-4 rounded-2xl border ${theme.cardBorder} ${isDarkMode ? 'bg-white/5' : 'bg-white'}`}>
                      <div className="flex items-center gap-2 mb-1 opacity-50">
                         <HeartPulse size={16} className="text-rose-500" />
                         <span className={`text-[10px] font-black uppercase tracking-widest ${theme.text}`}>HR (2hr)</span>
                      </div>
                      <span className={`text-2xl font-black ${theme.text}`}>{extraDetails.avgHr || '--'} <span className="text-xs opacity-50">bpm</span></span>
                   </div>
                   <div className={`p-4 rounded-2xl border ${theme.cardBorder} ${isDarkMode ? 'bg-white/5' : 'bg-white'}`}>
                      <div className="flex items-center gap-2 mb-1 opacity-50">
                         <Footprints size={16} className="text-emerald-500" />
                         <span className={`text-[10px] font-black uppercase tracking-widest ${theme.text}`}>Steps</span>
                      </div>
                      <span className={`text-2xl font-black ${theme.text}`}>{extraDetails.steps?.toLocaleString() || '0'}</span>
                   </div>
                   <div className={`col-span-2 p-4 rounded-2xl border ${theme.cardBorder} ${isDarkMode ? 'bg-white/5' : 'bg-white'}`}>
                      <div className="flex items-center gap-2 mb-1 opacity-50">
                         <Flame size={16} className="text-orange-500" />
                         <span className={`text-[10px] font-black uppercase tracking-widest ${theme.text}`}>Calories Burned</span>
                      </div>
                      <span className={`text-2xl font-black ${theme.text}`}>{extraDetails.calories?.toLocaleString() || '0'} <span className="text-xs opacity-50">kcal</span></span>
                   </div>
                </div>
              ) : (
                <>
