...
interface FocusPageProps {
  ...
  onUpdateStudyTime?: (hours: number) => void;
}
...
const FocusPage: React.FC<FocusPageProps> = ({ isDarkMode, theme, focusState, setFocusState, onUpdateStudyTime }) => {
  ...
  // Time accumulation ref to batch updates
  ...
  // Timer Interval Effect hooked to global tracking
  useEffect(() => {
    ...
    if (isActive && timeLeft > 0) {
      interval = setInterval(() => {
        ...
        // Every 60 seconds, update global state
        if (accumulatedTimeRef.current >= 60) {
            if (onUpdateStudyTime) {
                onUpdateStudyTime(1/60); // Add 1 minute (in hours)
            }
    ...
    return () => clearInterval(interval);
  }, [isActive, timeLeft, onUpdateStudyTime]);
